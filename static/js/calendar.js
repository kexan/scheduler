class Calendar {
  ...
  render(slots) {
    ...
    for (let i = firstDay - 1; i >= 0; i--) {
      ...
      const dateStr = this.getDateString(prevDay, this.currentMonth - 1, this.currentYear);
      html += `<div class="calendar-day other-month">
                <div class="calendar-day-number">${prevDay}</div>
                <div class="calendar-slots">${this.renderDaySlots([], dateStr)}</div>
            </div>`;
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      html += `<div class="${dayClass}" data-date="${dateStr}">
                ...
                ` : ''}
                <div class="calendar-day-number ${isCurrentOrPast ? 'disabled-day' : ''}">${day}</div>
    ...
    for (let day = 1; day <= nextMonthDays; day++) {
      const dateStr = this.getDateString(day, this.currentMonth + 1, this.currentYear);
      html += `<div class="calendar-day other-month">
  ...
  renderDaySlots(slots, dateStr) {
    ...
    if (this.isAdminMode) {
      html += `<div class="calendar-slot admin-add-slot" data-date="${dateStr}" title="Добавить слот">
                +
            </div>`;
    }
    ...
    if (slots.length === 0) {
      ...
      return '<div class="text-muted small">Нет слотов</div>';
    }

    const displaySlots = slots.slice(0, 4);
    const remainingSlots = slots.length - 4;

    html += displaySlots.map(slot => {
      let content = '';
    ...
    if (remainingSlots > 0) {
      html += `<div class="calendar-slot-more" data-date="${dateStr}">
                +${remainingSlots} еще
            </div>`;
    }
  ...
  updateCopyButton() {
    ...
    if (copyBtn) {
      copyBtn.disabled = !(this.selectedDaysForCopy.size > 0 && sourceDateValue);
    }
  }
  ...
  showCopyConfirmation(sourceDate, sourceSlots, targetDates) {
    ...
    DOMHelper.html('copyTargetDates', targetDates.map(date =>
  ...
  async performCopy(sourceSlots, targetDates) {
    ...
    ButtonHelper.setState('toggleCopyPanel', '📋 Копирование расписания', 'btn-outline-danger', 'btn-outline-info');
  ...
  attachCalendarSlotListeners() {
    document.querySelectorAll('.calendar-slot:not(.admin-add-slot)').forEach(slot => {
      slot.addEventListener('click', (e) => {
        ...
        if (this.isAdminMode) {
          ...
          Utils.showError('Нельзя записываться на слоты в текущие и прошедшие даты');
        } else {
    ...
    document.querySelectorAll('.admin-add-slot').forEach(addBtn => {
    ...
    document.querySelectorAll('.calendar-slot-more').forEach(more => {
  ...
  showDayDetails(dateStr) {
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Слоты на ${Utils.formatDate(dateStr)}</h5>
    ...
    modal.querySelectorAll('.slot-action-btn').forEach(button => {
  ...
  renderSlot(slot) {
    ...
    const buttonText = canBook ? 'Записаться' : (slot.is_available ? 'Недоступно для записи' : 'Занят');
