class Scheduler {
  ...
  setupEventListeners() {
    ...
    if (toggleBtn) {
      EventManager.onClick('toggleCopyPanel', () => {
        ...
        if (!isVisible) {
          ButtonHelper.setState('toggleCopyPanel', '❌ Закрыть копирование', 'btn-outline-info', 'btn-outline-danger');
    ...
    EventManager.onClick('confirmCopy', async () => await this.calendar.copySchedule());
    EventManager.onSubmit('copyForm', async () => await this.calendar.copySchedule());
  ...
  async loadSlots() {
    try {
      this.slots = await this.api.getSlots();
      this.calendar.render(this.slots);
    } catch (error) {
      console.error('Ошибка загрузки слотов:', error);
      Utils.showError('Не удалось загрузить слотов');
    }
  }
